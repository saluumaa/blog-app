...
RSpec.describe "Post show page", type: :feature do
  ...
  let!(:comment1) { Comment.create(text: 'First comment', author: user, post: post) }
  let!(:comment2) { Comment.create(text: 'Second comment', author: user, post: post) }
  let!(:like1) { Like.create(author: user, post: post) }
  let!(:like2) { Like.create(author: user, post: post) }
